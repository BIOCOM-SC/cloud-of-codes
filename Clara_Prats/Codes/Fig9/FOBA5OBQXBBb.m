function     [BOBQ,BerrOBQ,R0OBQ,R0errOBQ,DayOBQ,DayOBQ20] = FOBA5OBQXBBb(N_Desc, N_Asc1, N_Asc2, N_Asc4, N_Ctt, Country, WeekThurs)
...
% Preparem on comencem el fitting, perquè sigui automàtic
            ...
            if (Country == "Italy" || Country == "Austria")
                principi = princ1 - 2;
            elseif (Country == "Denmark" || Country == "Croatia" || Country == "Luxembourg" || Country == "Portugal"|| Country=="Romania")
                principi = princ1 - 3;
...
% i on l'acabem!!
final2 = 0;
final1 = find(Norm_Asc2>70,1);
% Preparem on comencem el fitting, perquè sigui automàtic
            try
                while final2 == 0
                final2 = Norm_Asc2(final1)>70 && Norm_Asc2(final1)>Norm_Asc2(final1+1) && Norm_Asc2(final1+1)>70;
                final1 = final1+1;
                end
            final = final1-1; %recuperem l'ultim valor que hem analitzat  
            catch
                    final = find(Norm_Asc2 == max(Norm_Asc2),1);
            end
...
Fitting_date=Day_plot(ID_fitting:final);
        ...
        global_fit_data = [Fit_Asc1;Fit_Asc2;Fit_Asc4;Fit_Ctt]'; %BQ.1; XBB; altres; BA.2-like
...
fit = fminsearch(squared_errors,[aa ab ac ad ae af ag ah],options); % BetaBQ, BetaBA2, BetaBA4, Beta_Altres; Ratio_ini_BQ, Ratio_ini_BA2, Ratio_ini_BA4, Ratio_ini_Altres
                ...
                if (fit(5) > 0.005 && fit(2)>0 && fit(4)>0)
    n=n+1;
    close
...
T = table(Fitting_date(1), paramfit(1), abs(conf1(1,1)-paramfit(1)), paramfit(5), abs(conf1(5,1)-paramfit(5)),...
    ...
    "Beta_OXBB","error_B_OXBB","R0_OXBB","error_R0_OXBB",...
    "Beta_BA2like","error_B_BA2like","R0_BA2like","error_R0_BA2like","p-value","RMSE"])
...
PlotSeqVar_oBA5_oBQ.Properties.VariableNames = ['Dia',"Perc_oBA5",'Error_oBA5',"Perc_oBQ",'Error_oBQ',"Perc_oXBB",'Error_oXBB',"Perc_altres",'Error_altres',"Perc_BA2like","Error_BA2like"];
...
errorbar(Day_plot,Norm_Asc2,Asc2_int,Asc2_int,'o','Color',[0.7 0.7 0.7])
errorbar(Day_plot,Norm_Asc4,Asc4_int,Asc4_int,'o','Color',[0.93 0.69 0.13])
errorbar(Day_plot,Norm_Ctt,Ctt_int,Ctt_int,'o','Color',[0.2 0.8 0.20])
...
plot(Fitting_date_plot,Fitplot_Asc2,'-','Color',[0.7 0.7 0.7])
plot(Fitting_date_plot,Fitplot_Asc4,'-','Color',[0.93 0.69 0.13])
plot(Fitting_date_plot,Fitplot_Ctt,'-','Color',[0.2 0.8 0.20])
...
fill([Fitting_date_plot flip(Fitting_date_plot)], [Fitplot_Asc2MIN flip(Fitplot_Asc2MAX)],'r','FaceColor',[0.7 0.7 0.7],'FaceAlpha',0.3,'EdgeColor','none');
fill([Fitting_date_plot flip(Fitting_date_plot)], [Fitplot_Asc4MIN flip(Fitplot_Asc4MAX)],'r','FaceColor',[0.93 0.69 0.13],'FaceAlpha',0.3,'EdgeColor','none');
fill([Fitting_date_plot flip(Fitting_date_plot)], [Fitplot_CttMIN flip(Fitplot_CttMAX)],'r','FaceColor',[0.2 0.8 0.20],'FaceAlpha',0.3,'EdgeColor','none');
yline(5,'--'); 
...
Fit_oBA5_oBQ.Properties.VariableNames = ['Dia',"Perc_oBA5_dia","Perc_oBA5_dia_errMAX","Perc_oBA5_dia_errMIN","Perc_oBQ_dia","Perc_oBQ_dia_errMAX","Perc_oBQ_dia_errMIN",...
    "Perc_oXBB_dia","Perc_oXBB_dia_errMAX","Perc_oXBB_dia_errMIN","Perc_altres_dia","Perc_altres_dia_errMAX","Perc_altres_dia_errMIN","Perc_BA2like_dia","Perc_BA2like_dia_errMAX","Perc_BA2like_dia_errMIN"];
Fitname = ".\DadesCSV_hist_variants_OBQ\Fit_SeqVar_oBA5_oBQ_oBA2_altres"+Country+".csv";
writetable(Fit_oBA5_oBQ,Fitname)

textCaption = ".\Figures_hist_variants_OBQ\" + Country + " oBA5 VS oBQ + oXBB + Altres + oBA2-like " + ID_fitting;
lgd = legend ('BA.5','BQ.1','XBB','Others','BA.2-like','Model BA.5','Model BQ.1','Model XBB','Linear Other','Model BA.2-like','Location','west');
lgd.FontSize = 7;
title(Country + " oBA5 VS oBQ + oXBB")
