function     [BOmicron,BerrOmicron,R0Omicron,R0errOmicron,DayOmicron,DayOmicron20,DayOmicron40,DayOmicron60,DayOmicron80] = FDeltaOmicron(N_Desc, N_Asc1, N_Ctt,Country,WeekThurs)
...
% Preparem on comencem el fitting, perquè sigui automàtic
        ...
        if Country == "Denmark" 
            principi = principi -5;
        elseif Country=="Estonia" || Country == "Iceland" || Country == "Ireland"
            principi = principi - 2;
...
fit = fminsearch(squared_errors,[aa ab ac ad],options); % BetaBA4, BetaBA5; Ratio_ini_BA4, Ratio_ini_BA5
                ...
                if (fit(3) > 0.003 && fit(1)>fit(2) && fit(2) > 0) %original fit(3) > 0.003 
    n=n+1;
...
T = table(Fitting_date(1), paramfit(1), abs(conf1(1,1)-paramfit(1)), paramfit(3), abs(conf1(3,1)-paramfit(3)),...
    ...
    'VariableNames',['Inici Fit',"Beta_Omicron","error_B_Omicron","R0_Omicron","error_R0_Omicron","Beta_Altres","error_B_Altres","R0_Altres","error_R0_Altres","p-value","RMSE"])
Tname = ".\Variables_hist_variants_Omicron\BetaR0_DeltaVSOmicron_altres"+Country+".xlsx";
...
PlotSeqVar_delta_omicron = table(Day_plot', Norm_Desc, Desc_int, Norm_Asc1, Asc1_int, Norm_Ctt, Ctt_int);
PlotSeqVar_delta_omicron.Properties.VariableNames = ['Dia',"Perc_delta",'Error_delta',"Perc_omicron",'Error_omicron',"Perc_altres","Error_altres"];
Plotname = ".\DadesCSV_hist_variants_Omicron\Plot_SeqVar_delta_omicron_altres" + Country + ".csv";
writetable(PlotSeqVar_delta_omicron,Plotname)

% Preparem la figura: dades SIVIC, observacions ("reals")
figure
errorbar(Day_plot,Norm_Desc,Desc_int,Desc_int,'o','Color',[0.85 0.33 0.10])
hold on
errorbar(Day_plot,Norm_Asc1,Asc1_int,Asc1_int,'o','Color',[0.13, 0.55, 0.13])
...
plot(Fitting_date_plot,Fitplot_Asc1,'-','Color',[0.13, 0.55, 0.13])
...
fill([Fitting_date_plot flip(Fitting_date_plot)], [Fitplot_Asc1MIN flip(Fitplot_Asc1MAX)],'r','FaceColor',[0.13, 0.55, 0.13],'FaceAlpha',0.3,'EdgeColor','none');
...
Fit_delta_omicron = table(Fitting_date_plot', Fitplot_Desc',Fitplot_DescMAX',Fitplot_DescMIN', Fitplot_Asc1',Fitplot_Asc1MAX',Fitplot_Asc1MIN',Fitplot_Ctt',Fitplot_CttMAX',Fitplot_CttMIN');
Fit_delta_omicron.Properties.VariableNames = ['Dia',"Perc_delta_dia","Perc_delta_dia_errMAX","Perc_delta_dia_errMIN","Perc_omicron_dia","Perc_omicron_dia_errMAX","Perc_omicron_dia_errMIN","Perc_altres_dia","Perc_altres_dia_errMAX","Perc_altres_dia_errMIN"];
Fitname = ".\DadesCSV_hist_variants_Omicron\Fit_SeqVar_delta_omicron_altres"+Country+".csv";
writetable(Fit_delta_omicron,Fitname)

textCaption = ".\Figures_hist_variants_Omicron\" + Country + " Delta VS omicron + Altres " + ID_fitting;
lgd = legend ('Delta','Omicron','Others','Model Delta','Model Omicron','Model Others','Location','best');
lgd.FontSize = 7;
title(Country + " Delta VS Omicron")
...
T = table(Fitting_date(1), paramfit(1), abs(conf1(1,1)-paramfit(1)), paramfit(3), abs(conf1(3,1)-paramfit(3)),p, RMSE,...
    'VariableNames',['Inici Fit',"Beta_Omicron","error_B_Omicron","R0_Omicron","error_R0_Omicron","p-value","RMSE"])
Tname = ".\Variables_hist_variants_Omicron\BetaR0_DeltaVSOmicron_altres"+Country+".xlsx";
NameSheet = "Full ID " + ID_fitting;
writetable(T,Tname,"Sheet",NameSheet);
...
textCaption = ".\Figures_hist_variants_Omicron\" + Country + " Delta VS omicron + Altres " + ID_fitting;
lgd = legend ('Delta','Omicron','Others','Model Delta','Model Omicron','Linear Others','Location','best');
lgd.FontSize = 7;
title(Country + " Delta VS Omicron")
xlim([Day_plot(ID_fitting)-1 Day_plot(final)+1]);
print(textCaption,'-dpng','-r300')
    ...
    BOmicron=paramfit(1); BerrOmicron=abs(conf1(1,1)-paramfit(1)); R0Omicron=paramfit(3); R0errOmicron=abs(conf1(3,1)-paramfit(3));
    DayOmicron = Fitting_date_plot(find(Fitplot_Asc1 > 5,1));
    DayOmicron20 = Fitting_date_plot(find(Fitplot_Asc1 > 20,1));
        DayOmicron40 = Fitting_date_plot(find(Fitplot_Asc1 > 40,1));
    DayOmicron60 = Fitting_date_plot(find(Fitplot_Asc1 > 60,1));
        DayOmicron80 = Fitting_date_plot(find(Fitplot_Asc1 > 80,1));
            close all
end