function     [BOBA2,BerrOBA2,R0OBA2,R0errOBA2,DayOBA2,DayOBA220,DayOBA240,DayOBA260,DayOBA280] = FOBA1OBA2b(N_Desc, N_Asc1, N_Ctt,Country,WeekThurs)
...
% Preparem on comencem el fitting, perquè sigui automàtic
        ...
        if Country == "Croatia" || Country =="Europe"
            principi = princ1-0;
        elseif Country == "Spain" || Country =="Bulgaria" || Country =="Slovenia"
            principi = princ1-2;
        elseif (Country == "Czechia" || Country == "Ireland")
            principi = princ1-3;
...
% Preparem on comencem el fitting, perquè sigui automàtic
        try
        while final2 == 0
                final2 = (Norm_Asc1(final1)>80 && Norm_Asc1(final1)>Norm_Asc1(final1+1) && Norm_Asc1(final1+1)>80) || (Norm_Asc1(final1)>95 && Norm_Asc1(final1+1)<96);
                final1 = final1+1;
        end
...
% ajustant:
aa=0.6;ab=0.6;ac=0.01;ad=0.0002;
...
fit = fminsearch(squared_errors,[aa ab ac ad],options); % BetaBA4, BetaBA5; Ratio_ini_BA4, Ratio_ini_BA5
                if (fit(3) > 0.005 && fit(1)>fit(2) && fit(2) > 0)
    n=n+1;
    close
...
T = table(Fitting_date(1), paramfit(1), abs(conf1(1,1)-paramfit(1)), paramfit(3), abs(conf1(3,1)-paramfit(3)),...
    ...
    'VariableNames',['Inici Fit',"Beta_OBA2","error_B_OBA2","R0_OBA2","error_R0_OBA2","Beta_Altres","error_B_Altres","R0_Altres","error_R0_Altres","p-value","RMSE"])
Tname = ".\Variables_hist_variants_OBA2\BetaR0_OBA1VSOBA2_altres"+Country+".xlsx";
...
PlotSeqVar_oBA1_oBA2 = table(Day_plot', Norm_Desc, Desc_int, Norm_Asc1, Asc1_int, Norm_Ctt, Ctt_int);
PlotSeqVar_oBA1_oBA2.Properties.VariableNames = ['Dia',"Perc_oBA1",'Error_oBA1',"Perc_oBA2",'Error_oBA2',"Perc_altres","Error_altres"];
Plotname = ".\DadesCSV_hist_variants_OBA2\Plot_SeqVar_oBA1_oBA2_altres" + Country + ".csv";
writetable(PlotSeqVar_oBA1_oBA2,Plotname)

% Preparem la figura: dades SIVIC, observacions ("reals")
figure
errorbar(Day_plot,Norm_Desc,Desc_int,Desc_int,'o','Color',[0.85 0. 0.7])
hold on
errorbar(Day_plot,Norm_Asc1,Asc1_int,Asc1_int,'o','Color',[0.2 0.8 0.20])
...
plot(Fitting_date_plot,Fitplot_Desc,'-','Color',[0.85 0. 0.7])
plot(Fitting_date_plot,Fitplot_Asc1,'-','Color',[0.2 0.8 0.20])
plot(Fitting_date_plot,Fitplot_Ctt,'-','Color',[0.93 0.69 0.13])

fill([Fitting_date_plot flip(Fitting_date_plot)], [Fitplot_DescMIN flip(Fitplot_DescMAX)],'r','FaceColor',[0.85 0. 0.7],'FaceAlpha',0.3,'EdgeColor','none');
fill([Fitting_date_plot flip(Fitting_date_plot)], [Fitplot_Asc1MIN flip(Fitplot_Asc1MAX)],'r','FaceColor',[0.2 0.8 0.20],'FaceAlpha',0.3,'EdgeColor','none');
...
Fit_oBA1_oBA2 = table(Fitting_date_plot', Fitplot_Desc',Fitplot_DescMAX',Fitplot_DescMIN', Fitplot_Asc1',Fitplot_Asc1MAX',Fitplot_Asc1MIN',Fitplot_Ctt',Fitplot_CttMAX',Fitplot_CttMIN');
Fit_oBA1_oBA2.Properties.VariableNames = ['Dia',"Perc_oBA1_dia","Perc_oBA1_dia_errMAX","Perc_oBA1_dia_errMIN","Perc_oBA2_dia","Perc_oBA2_dia_errMAX","Perc_oBA2_dia_errMIN","Perc_altres_dia","Perc_altres_dia_errMAX","Perc_altres_dia_errMIN"];
Fitname = ".\DadesCSV_hist_variants_OBA2\Fit_SeqVar_oBA1_oBA2_altres"+Country+".csv";
writetable(Fit_oBA1_oBA2,Fitname)

textCaption = ".\Figures_hist_variants_OBA2\" + Country + " oBA1 VS oBA2 + Altres " + ID_fitting;
lgd = legend ('o(BA1)','o(BA2)','Others','Model o(BA1)','Model o(BA2)','Model Others','Location','best');
lgd.FontSize = 7;
title(Country + " o(BA1) VS o(BA2)")
xlim([Day_plot(ID_fitting)-1 Day_plot(final)+1]);
print(textCaption,'-dpng','-r300')
close

                elseif ((fit(3) > 0.005 && fit(1)<fit(2)) || (fit(3) > 0.005 && fit(2) < 0))
                disp("*********************is working!!****************")
                    global_fit_data = [Fit_Asc1;Fit_Ctt]'; 
...
T = table(Fitting_date(1), paramfit(1), abs(conf1(1,1)-paramfit(1)), paramfit(3), abs(conf1(3,1)-paramfit(3)),p, RMSE,...
    'VariableNames',['Inici Fit',"Beta_OBA2","error_B_OBA2","R0_OBA2","error_R0_OBA2","p-value","RMSE"])
Tname = ".\Variables_hist_variants_OBA2\BetaR0_OBA1VSOBA2_altres"+Country+".xlsx";
NameSheet = "Full ID " + ID_fitting;
writetable(T,Tname,"Sheet",NameSheet);
...
textCaption = ".\Figures_hist_variants_OBA2\" + Country + " oBA1 VS oBA2 + Altres " + ID_fitting;
lgd = legend ('o(BA1)','o(BA2)','Others','Model o(BA1)','Model o(BA2)','Linear Others','Location','best');
lgd.FontSize = 7;
title(Country + " o(BA1) VS o(BA2)")
xlim([Day_plot(ID_fitting)-1 Day_plot(final)+1]);
print(textCaption,'-dpng','-r300')
close
    ...
    BOBA2=paramfit(1); BerrOBA2=abs(conf1(1,1)-paramfit(1)); R0OBA2=paramfit(3); R0errOBA2=abs(conf1(3,1)-paramfit(3));
        DayOBA2 = Fitting_date_plot(find(Fitplot_Asc1 > 5,1));
            DayOBA220 = Fitting_date_plot(find(Fitplot_Asc1 > 20,1));
        DayOBA240 = Fitting_date_plot(find(Fitplot_Asc1 > 40,1));
    DayOBA260 = Fitting_date_plot(find(Fitplot_Asc1 > 60,1));
        DayOBA280 = Fitting_date_plot(find(Fitplot_Asc1 > 80,1));
