...
# Nivells: basal-threshold-baix-0.25-moderat-0.5-mitja-0.75-alt-0.95-molt alt
...
EpiLevels<-function(df,desired.quantiles=c(.25,.50,.75,.95)){ # df ?s un excel on les columnes son les temporades
  ...
  didt<-diff(mean_evol)
  first_local_maximum<-which((didt)>=2)[1]
  ...
  g2<-ggplot(df3,aes(x=time,y=value))+
    geom_line()+
    theme_classic()+
    geom_vline(xintercept = first_local_maximum,lty='dashed')+
    labs(x='time')+
...
library(xlsx)
numero_fulla_excel = 14 # 6 for influenza, 14 for bronchiolitis 
